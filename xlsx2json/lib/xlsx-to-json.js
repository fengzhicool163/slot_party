    ...
    for (var i_sheet = 0; i_sheet < excel.worksheets.length; i_sheet++) {
        ...
        if (sheet.data && sheet.data.length > 0) {
            ...
            for (var i_cell = 0; i_cell < row_head.length; i_cell++) {
                if(!row_head[i_cell]){
                    console.error('row_head:',row_head,' no ',i_cell);
                    continue;
                }
            ...
            var writeData;
            if(isObj){
                writeData = JSON.stringify(outObj);
            } else {
                output = JSON.stringify(output, null, 2);
                writeData = output;
            }
            saveFile(dest, sheet.name + ".json", writeData);
        }
    }
}

function saveFile(directory, fileName, writeData) {
    var dest_file = path.resolve(directory, fileName);
    fs.writeFile(dest_file, writeData, function(err) {
        if (err) {
